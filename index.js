...
// Safely import analytics_events.json if available
let analyticsEventsData = {};
try {
    analyticsEventsData = require('./analytics_events.json');
} catch (e) {
    console.log('analytics_events.json not found, using proxy for analytics');
}
...
// MoEngage SDK Config Proxy Endpoint
app.all('/v3/sdkconfig/android/YK7OBFDSPNTUQ9KQUQHSO4NO', async (req, res) => {
    try {
        const targetUrl = 'https://sdk-03.moengage.com/v3/sdkconfig/android/YK7OBFDSPNTUQ9KQUQHSO4NO';
...
app.post('/v2/analytics/send/storymax/events', async (req, res) => {
    try {
        const response = await fetch(
            'https://central-analytics-producer.eloelo.in/v2/analytics/send/storymax/events',
            {
                method: 'POST',
                headers: {
                    'Authorization': 'Bearer 1785705059075',
                    'platform': '0',
